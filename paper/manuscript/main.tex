...
\hypersetup{
  pdftitle={Stress-Testing Data Minimization Across Review Budgets and Decision Stages in Educational Early Warning},
  pdfauthor={Aigul Shaikhanova; Oleksandr Kuznetsov; Gulmira Shangytbayeva; Kamila Bakenova; Kainizhamal Iklassova; Dana Tulemisova},
  pdfsubject={Post-development multi-budget, proxy-persistence, and harmonized exclusion audit in OULAD and two UCI datasets},
  pdfkeywords={learning analytics, early-warning systems, data minimization, workload, proxy persistence, multiplicity, temporal validation, teacher decision support}
}
...
\title{Stress-Testing Data Minimization Across Review Budgets and Decision Stages in Educational Early Warning:\\Paired Evidence from OULAD and Two UCI Datasets}

\renewcommand{\Affilfont}{\fontsize{8.5}{10.2}\selectfont}
\renewcommand{\Authfont}{\normalsize}
\setlength{\affilsep}{0.4em}
\author[1,2]{Aigul Shaikhanova}
\author[3,4]{Oleksandr Kuznetsov}
\author[5]{Gulmira Shangytbayeva}
\author[1,6]{Kamila Bakenova}
\author[7]{Kainizhamal Iklassova}
\author[8]{Dana Tulemisova}
\affil[1]{Global Education and Training, iSchool at Illinois, University of Illinois at Urbana-Champaign, Champaign, IL 61820, USA}
\affil[2]{Department of Information Security, L.N. Gumilyov Eurasian National University, Satpayev 2, 010008 Astana, Kazakhstan}
\affil[3]{Department of Theoretical and Applied Sciences, eCampus University, Via Isimbardi 10, 22060 Novedrate (CO), Italy}
\affil[4]{Department of Intelligent Software Systems and Technologies, School of Computer Science and Artificial Intelligence, V.~N.~Karazin Kharkiv National University, 4 Svobody Sq., 61022 Kharkiv, Ukraine}
\affil[5]{Department of Informatics and Information Technologies, Zhubanov University, 34 Aliya Moldagulova Avenue, 030000 Aktobe, Kazakhstan}
\affil[6]{Center for Scientific Research and Commercialization, Arkalyk Pedagogical University, 17 Auelbekov str., 110300 Arkalyk, Kazakhstan}
\affil[7]{Department of Information and Communication Technologies, Manash Kozybayev North Kazakhstan University, Pushkin Str.~86, 150000 Petropavlovsk, Kazakhstan}
\affil[8]{School of Cybersecurity, Astana IT University, 55/11 Mangilik El Avenue, 010000 Astana, Kazakhstan}
\date{Working revision v0.6.0, 2 September 2026\\\small Post-development exploratory analysis; authorship, funding, ethics, and final author approval remain to be confirmed}

\begin{document}
\maketitle

\begin{abstract}
Data-minimization claims for educational early-warning systems may change with review capacity, prediction stage, excluded field block, and evaluation metric. We extended a provenance-corrected analysis of the Open University Learning Analytics Dataset (OULAD) and two separately identified UCI datasets without refitting the frozen legacy models. Saved predictions were re-evaluated at 5\%, 10\%, 20\%, and 30\% within-group review budgets. We also tested whether directly excluded sex/gender and disability/special-needs fields remained predictable from retained inputs, and fitted only the missing histogram-gradient-boosting models needed for two operationally harmonized blocks: sex/gender and socioeconomic/family information. New restricted models were accepted only when a same-run full control reproduced its frozen probability to within $10^{-12}$. Paired cluster or row bootstraps used 5,000 primary replicates, with Bonferroni-percentile intervals across all frozen stages and budgets. Of 296 primary precision cells, 30 had family-wise intervals below zero and none above zero; this count includes planned duplicate views of exact pairs and is not 30 independent confirmations. Sex/gender exclusion had no family-wise workload-precision difference in any dataset. Socioeconomic/family exclusion reduced global predictive performance at every OULAD horizon and both UCI 697 stages, while workload penalties were concentrated at selected OULAD 30\% queues and were large at UCI 697 enrollment. UCI 320 workload differences remained inconclusive. Fixed probes predicted excluded fields above chance, but this establishes residual predictability, not actual proxy use by the outcome models. The results support stage-, capacity-, block-, and metric-specific governance rather than a universal claim that direct exclusion is cost-free. They are post-development, conditional on fitted models, and do not establish equivalence, fairness, privacy, or causal educational benefit.
\end{abstract}

\noindent\textbf{Keywords:} learning analytics; early-warning systems; data minimization; workload-constrained evaluation; proxy persistence; multiplicity; temporal validation; teacher decision support
...
\begin{tabular}{lrrrr}
\toprule
Metric & Gender/disability excluded & Full & Difference & 95\% interval \\
\midrule
Alerts & 908 & 908 & --- & --- \\
...
\footnotesize
\item Intervals are paired student-cluster percentile intervals with 5,000 replicates, conditional on locked models. Positive Brier differences favor the full model.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
The favorable precision sensitivity was metric-specific. At day 14, average precision decreased by $-0.00342$ (Bonferroni interval $[-0.00613,-0.00071]$). At the later five horizons, the familywise average-precision intervals included zero. At day 42, the small AUROC difference was $-0.0018$ (pointwise interval $[-0.0027,-0.0009]$) and the Brier difference was $+0.0006$ ($[0.0003,0.0008]$), both favoring full.
...
The restricted-minus-full findings were much less stable. Nearly all UCI 320 HGB pointwise and within-policy familywise intervals included zero for AP, AUROC, Brier, and workload precision. The sole HGB contrast whose three-stage familywise interval excluded zero was a small Mathematics period-one AUROC improvement after removing sex ($+0.0050$). Because this isolated result sits among multiple subjects, policies, and metrics, it is retained as a non-confirmatory finding. UCI 320 therefore supports the informational value of staged grades, not a precise or universal exclusion-cost trajectory.
...
OULAD is available from the UCI Machine Learning Repository at
\url{https://archive.ics.uci.edu/dataset/349/open%2Buniversity%2Blearning%2Banalytics%2Bdataset}
and is described by \citet{kuzilek2017oulad}. The official archive used here had SHA-256
\texttt{\seqsplit{f2ed1902616c1fe8d2824d872c0b7d2d72be435bf0124d077044fe4be2c6d3e4}}.
UCI 697 is available at \url{https://archive.ics.uci.edu/dataset/697/predict+students+dropout+and+academic+success}; the outer ZIP used here had SHA-256 \texttt{\seqsplit{e90e55fd65ec462ae283ebeb2cca409319e3460ed898d8754f62fb35cc83a65d}}. UCI 320 is available at \url{https://archive.ics.uci.edu/dataset/320/student+performance}; the outer ZIP had SHA-256 \texttt{\seqsplit{82ae9d66437b9808df42e8c89d2bb179c46e9cfbcf06f38abc1d20b3b747e177}}. The current nested \texttt{student.zip}, not \texttt{.student.zip\_old}, was analyzed. All three UCI pages state CC BY 4.0 licenses. Member-level hashes and input audits are retained with the computational checkpoints.

\section*{Code and Materials Availability}

The internal v0.6.0 checkpoint contains the canonical C13E project; frozen C14 protocol; multi-budget tables from saved predictions; 44 proxy-probe bundles; 28 new harmonized HGB bundles with full-control replay; 42 paired-bootstrap draw files; machine-readable interval tables; verification reports; and manuscript source. Separate verification-only and rebuild-from-raw paths do not overwrite immutable reference results. Public repository and archival identifiers do not yet exist. Distribution of raw or row-level records and fitted models must be reviewed against dataset licensing, institutional policy, and privacy obligations before public release.

\section*{Supplementary Materials}

The separate supplement reports the evidence ledger, predictor inventories, complete OULAD sensitivities, UCI policy tables, historical replay, negative alternatives, C14 primary families, all harmonized 10\% contrasts, proxy-probe metrics, and the extended reproducibility inventory.

\section*{Ethics Statement}

This retrospective computational study used a publicly accessible secondary dataset and involved no recruitment, contact, intervention, re-identification attempt, or access to non-public records. Sensitive/background fields were used in the full comparator and for descriptive auditing. The applicable institutional determination regarding exemption or waiver must be confirmed before submission. Predictive outputs must not be used as automated eligibility or sanction decisions and require human review and prospective governance.

\section*{Funding -- confirmation required}

The working author metadata associates this project with the Science Committee of the Ministry of Science and Higher Education of the Republic of Kazakhstan (Grant No. AP23489228). The responsible authors and institution must confirm in writing that the grant applies to this specific study and approve the exact wording before submission.

\section*{Author Contributions -- confirmation required}

The author list and affiliations follow supplied working metadata. Individual CRediT roles, author order, corresponding-author designation, affiliations, and approval of the complete manuscript must be confirmed directly by every listed author before submission.

\section*{Competing Interests -- confirmation required}

No final declaration is made in this working draft. Every listed author must provide and approve a competing-interest declaration before submission.

\section*{Use of AI-Assisted Tools}

AI-assisted tools supported computational development, checking, visualization, and drafting of this working revision. The authors must independently inspect the code, numerical results, references, and final text; document any corrections; and adapt this disclosure to the target venue's policy before approving submission. These activities are not represented as language editing alone.
